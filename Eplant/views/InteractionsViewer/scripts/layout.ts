  ...
  if (loadFlags.existsPDI || proteinNodes.length < 100) {
    ...
    const layout = proteinNodes
      .layout({
        ...
        // @ts-expect-error error with fit, even when documentation says it should work
        fit: false,
 ...
 * @return {void}
 */
const positionProteinBack = (nodes: NodeCollection) => {
 ...
 * @return {void}
 */
const transformAverage = (nodes: NodeCollection, offset: boolean) => {
  let minX = 10000
  let maxY = -10000
  let minY = 10000
  ...
  // Transform positions
  nodes.positions((node: NodeSingular, i: number) => {
 ...
 */
const positionDNALinear = (numNodes: number) => {
  const x = -350
  const midpoint = 0
 ...
 */
const positionDNACircular = (numNodes: number) => {
